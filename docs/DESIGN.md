...
| `waitStream` | 스트리밍이나 진행 상태가 끝나는 조건까지 기다린다. 15초를 넘으면 `compose`에서 해당 구간만 배속 처리하고 "배속" 표시를 넣는다 | 완료 시점에 결과 `highlight` |
...
| FR-045 | 총 길이가 90초를 넘으면 `validate`가 경고하고 분할을 제안한다. 120초를 넘으면 실패한다 |
